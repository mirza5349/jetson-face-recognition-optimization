...
def run_benchmark(config_path, hw_config_path, num_frames=50, mock=False):
    ...
    print("=" * 60)
    print(f"  BENCHMARK - TENSORRT PIPELINE (FP16 Engine)")
    print("=" * 60)
    
    # Initialize hardware stats monitor
    monitor = JetsonStatsMonitor(interval_ms=100, mock=mock)
    monitor.set_workload_context(backend="tensorrt", batch_size=1)
    monitor.start()

    # Initialize TensorRT Pipeline
    engine_path = h_cfg["paths"]["tensorrt_path"]
    pipeline = TensorRTFaceNetPipeline(device="cuda", model_path=engine_path, mock=mock)

    # Prepare dummy database for Cosine matching
    db_size = 100
    ...
    # Warm-up iterations (Essential for TensorRT context stabilization)
    print("Warming up compiled context (15 warm-up iterations)...")
    for _ in range(15):
    ...
    out_path = "results/raw/benchmark_tensorrt.json"
...
def main():
    parser = argparse.ArgumentParser(description="Benchmark FaceNet TensorRT FP16 Pipeline.")
