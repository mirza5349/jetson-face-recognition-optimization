...
def run_benchmark(config_path, hw_config_path, num_frames=50, mock=False):
    ...
    print("=" * 60)
    print(f"  BENCHMARK - ONNX RUNTIME PIPELINE (CUDA Provider)")
    print("=" * 60)
    
    # Initialize hardware stats monitor
    monitor = JetsonStatsMonitor(interval_ms=100, mock=mock)
    monitor.set_workload_context(backend="onnx", batch_size=1)
    monitor.start()

    # Initialize ONNX Pipeline
    device = "cuda" if (h_cfg["inference"]["use_gpu"] and not mock) else "cpu"
    onnx_path = h_cfg["paths"]["onnx_path"]
    
    pipeline = ONNXFaceNetPipeline(device=device, model_path=onnx_path, mock=mock)
    ...
    results = {
        "backend": "onnx_cuda",
        "precision": "fp32_onnx", # Standard ONNX export is usually FP32
        "resolution": "640x480",
        "batch_size": 1,
        "number_of_faces": 1,
        "pose_angle": 0,
    ...
    out_path = "results/raw/benchmark_onnx.json"
...
def main():
    parser = argparse.ArgumentParser(description="Benchmark FaceNet ONNX Runtime CUDA Pipeline.")
